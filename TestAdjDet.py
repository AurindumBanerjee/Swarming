# ============================================================
# Sequential PSO + Adjoint–Determinant Inversion (CPU)
# Baseline-Compatible Outputs, Logs, Metrics
# ============================================================

# cd /DATA/Aurindum/Swarming
# nohup python TestAdjDet.py > adjdet.log 2>&1 &
...
LOG_FILE = os.path.join(BASE_DIR, "sequential_adjoint_determinant.log")
...
# ---------------- LOGGING ----------------
def setup_logger():
    logger = logging.getLogger("cpu_adjdet")
...
def make_output_folder():
    ...
    path = os.path.join(
        BASE_OUTPUT_DIR,
        f"Method_Sequential_AdjointDeterminant_{ts}"
    )
...
def plot_global_convergence(histories, out_folder):
    ...
    plt.xlabel("Iteration")
    plt.ylabel("|Z11| (Ohms)")
    plt.title("Global Convergence — Adjoint–Determinant")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(ncol=2)
    plt.tight_layout()
    plt.savefig(
...
# ---------------- LOAD DATA ----------------
print("Loading PDN data (Adjoint–Determinant baseline)...")
...
def adjoint(A):
    n = A.shape[0]
    adj = np.zeros_like(A, dtype=A.dtype)
    for i in range(n):
        for j in range(n):
            adj[j, i] = ((-1)**(i+j)) * determinant(minor_matrix(A, i, j))
    return adj

# ---------------- FITNESS ----------------
def fitness_adjoint_det(x, n_caps):
    ...
    for i in range(x.shape[0]):
        ...
        for f in range(N_FREQS):
            ...
            adjY = adjoint(Yf)
            z11 = abs(adjY[0, 0] / detY)
            max_z11 = max(max_z11, z11)
...
def run_sequential():
    ...
    with open(cfg_path, "w") as f:
        ...
        for n_caps in range(1, MAX_CAPS + 1):
            ...
            cost, pos = opt.optimize(
                lambda x: fitness_adjoint_det(x, n_caps),
                iters=N_ITERATIONS,
                verbose=False
            )
    ...
    with open(os.path.join(out_folder, "metrics.txt"), "w") as f:
        ...
        f.write("Method: Adjoint–Determinant\n")

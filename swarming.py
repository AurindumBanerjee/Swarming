...
try:
    ...
    # Move to GPU
    y_base_gpu = cp.asarray(y_data_np)
    all_decap_admittances_gpu = cp.asarray(decaps_data_np)
    z_target_gpu = cp.asarray(Z_TARGET)
...
def fitness_function_dynamic(particle_batch_np, n_caps_current):
    ...
    # 2. Collision Resolution (CPU Side)
    # Mimics the logic: if a node is taken, move to next available
    ...
    # 3. GPU Evaluation
    # Note: We loop over particles here because creating a massive
    # (50, 1391, 21, 21) tensor often causes OOM on smaller GPUs.
    for i in range(n_particles):
        p_models = models_idx[i]
        p_locs = locs_idx[i]
        
        # Initialize diagonal matrix batch (Freq x Node x Node)
        y_decaps_gpu = cp.zeros((N_FREQS, N_NODES, N_NODES), dtype=complex)
        
        # Get admittances for chosen caps
        current_caps_gpu = all_decap_admittances_gpu[p_models, :]
        
        # Add to diagonals
        for k in range(n_caps_current):
            node_idx = p_locs[k]
            y_decaps_gpu[:, node_idx, node_idx] += current_caps_gpu[k]
        
        # Solve Circuit
        y_total = y_base_gpu + y_decaps_gpu
        try:
            z_total = cp.linalg.inv(y_total)
        except cp.linalg.LinAlgError:
            costs[i] = 1e200 # Penalty for singular matrix
            continue
        
        # Measure Port 1 (Index 0,0) Only
        z_at_port_1 = cp.abs(z_total[:, 0, 0])
        
        # Cost = Peak Impedance
        costs[i] = cp.asnumpy(cp.max(z_at_port_1))

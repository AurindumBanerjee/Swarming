# ============================================================
# Sequential PSO + Gaussian (Manual) Inversion (CPU)
# Baseline-Compatible Outputs, Logs, Metrics
# ============================================================

# cd /DATA/Aurindum/Swarming
# nohup python TestGauss.py > gauss.log 2>&1 &
...
LOG_FILE = os.path.join(BASE_DIR, "sequential_gaussian_baseline.log")
...
# ---------------- LOGGING ----------------
def setup_logger():
    logger = logging.getLogger("cpu_gaussian")
...
def make_output_folder():
    ...
    path = os.path.join(
        BASE_OUTPUT_DIR,
        f"Method_Sequential_GaussianInversion_{ts}"
    )
...
def plot_global_convergence(histories, out_folder):
    ...
    plt.xlabel("Iteration")
    plt.ylabel("|Z11| (Ohms)")
    plt.title("Global Convergence — Gaussian Inversion")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(ncol=2)
    plt.tight_layout()
    plt.savefig(
...
# ---------------- LOAD DATA ----------------
print("Loading PDN data (Gaussian Inversion baseline)...")
...
def fitness_gaussian(x, n_caps):
    ...
    for i in range(x.shape[0]):
        ...
        for f in range(N_FREQS):
            ...
            try:
                Zf = gaussian_inverse(Yf)
                max_z11 = max(max_z11, abs(Zf[0, 0]))
...
def run_sequential():
    ...
    with open(cfg_path, "w") as f:
        ...
        for n_caps in range(1, MAX_CAPS + 1):
            ...
            cost, pos = opt.optimize(
                lambda x: fitness_gaussian(x, n_caps),
                iters=N_ITERATIONS,
                verbose=False
            )
    ...
    with open(os.path.join(out_folder, "metrics.txt"), "w") as f:
        ...
        f.write("Method: Gaussian Manual Inversion\n")
